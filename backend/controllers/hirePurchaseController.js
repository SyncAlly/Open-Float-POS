...
async function getAgreements(req, res) {
  try {
    ...
    if (branch_id && branch_id !== 'all') { sql += ' AND branch_id = ?'; params.push(branch_id); }
...
async function createAgreement(req, res) {
  try {
    const db = await getDb();
    const { customer_name, customer_phone, item_name, total_value, down_payment, monthly_instalment, next_due, branch_id } = req.body;
    ...
    const result = exec(db,
      `INSERT INTO hire_purchase (ref, customer_name, customer_phone, item_name, total_value, down_payment, monthly_instalment, paid_instalments, balance, next_due, status, branch_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'active', ?)`,
      [ref, customer_name, customer_phone || '', item_name, total_value, down_payment, monthly_instalment || 0, balance, next_due || '2026-09-01', branch_id || null]
    );
